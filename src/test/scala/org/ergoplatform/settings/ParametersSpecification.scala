...
class ParametersSpecification extends ErgoPropertyTest {
  ...
  // todo remove
  val todo: Option[SoftForkParameters] = None
  ...
  property("simple voting - start - conditions") {
    ...
    val p: Parameters = Parameters(2, Map(StorageFeeFactorIncrease -> kInit, BlockVersion -> 0), None)
    ...
    val p4 = Parameters(4, Map(StorageFeeFactorIncrease -> (kInit + Parameters.StorageFeeFactorStep), BlockVersion -> 0), None)
  ...
  property("soft fork - w. activation") {
    ...
    val p: Parameters = Parameters(1, Map(BlockVersion -> 0), todo)
    val vr: VotingData = VotingData.empty
    ...
    val expectedParameters2 = Parameters(2, Map(SoftForkStartingHeight -> 2, SoftForkVotesCollected -> 0, BlockVersion -> 0), todo)
    ...
    val wrongParameters2 = Parameters(2, Map(BlockVersion -> 0), todo)
    ...
    val wrongParameters2a = Parameters(2, Map(SoftForkStartingHeight -> 2, SoftForkVotesCollected -> 1, BlockVersion -> 0), todo)
    esc1.process(h2, wrongParameters2a).isFailure shouldBe true

    // wrong parameters: invalid starting height
    val wrongParameters2b = Parameters(2, Map(SoftForkStartingHeight -> 4, SoftForkVotesCollected -> 0, BlockVersion -> 0), todo)
    esc1.process(h2, wrongParameters2b).isFailure shouldBe true

    // wrong parameters: incorrect block version
    val wrongParameters2c = Parameters(2, Map(SoftForkStartingHeight -> 2, SoftForkVotesCollected -> 0, BlockVersion -> 1), todo)
    ...
    val expectedParameters4 = Parameters(4, Map(SoftForkStartingHeight -> 2, SoftForkVotesCollected -> 2, BlockVersion -> 0), todo)
    ...
    val wrongParameters4 = Parameters(4, Map(BlockVersion -> 0), todo)
    esc3.process(h4, wrongParameters4).isFailure shouldBe true

    // wrong parameters: collected votes value is wrong
    val wrongParameters4a = Parameters(4, Map(SoftForkStartingHeight -> 2, SoftForkVotesCollected -> 3, BlockVersion -> 0), todo)
    esc3.process(h4, wrongParameters4a).isFailure shouldBe true

    // wrong parameters: invalid starting height
    val wrongParameters4b = Parameters(4, Map(SoftForkStartingHeight -> 3, SoftForkVotesCollected -> 2, BlockVersion -> 0), todo)
    ...
    val expectedParameters6 = Parameters(6, Map(SoftForkStartingHeight -> 2, SoftForkVotesCollected -> 4, BlockVersion -> 0), todo)
    val esc6 = esc5.process(h6, expectedParameters6).get

    // wrong parameters: voting is not reflected in the parameters
    val wrongParameters6 = Parameters(6, Map(BlockVersion -> 0), todo)
    esc5.process(h6, wrongParameters6).isFailure shouldBe true

    // wrong parameters: collected votes value is wrong
    val wrongParameters6a = Parameters(6, Map(SoftForkStartingHeight -> 2, SoftForkVotesCollected -> 5, BlockVersion -> 0), todo)
    esc5.process(h6, wrongParameters6a).isFailure shouldBe true

    // wrong parameters: invalid starting height
    val wrongParameters6b = Parameters(6, Map(SoftForkStartingHeight -> 4, SoftForkVotesCollected -> 2, BlockVersion -> 0), todo)
    ...
    val esc11 = (7 to 11).foldLeft(esc6) { case (esc, i) =>
      ...
      if (i % 2 == 0) {
        // wrong parameters: voting is not reflected in the parameters
        val wrongParametersI = Parameters(i, Map(BlockVersion -> 0), todo)
        esc.process(h, wrongParametersI).isFailure shouldBe true

        // wrong parameters: collected votes value is wrong
        val wrongParametersIa = Parameters(i, Map(SoftForkStartingHeight -> 2, SoftForkVotesCollected -> 5, BlockVersion -> 0), todo)
        esc.process(h, wrongParametersIa).isFailure shouldBe true

        // wrong parameters: invalid starting height
        val wrongParametersIb = Parameters(i, Map(SoftForkStartingHeight -> 4, SoftForkVotesCollected -> 4, BlockVersion -> 0), todo)
        esc.process(h, wrongParametersIb).isFailure shouldBe true

        // wrong parameters: invalid block version
        val wrongParametersIc = Parameters(i, Map(SoftForkStartingHeight -> 2, SoftForkVotesCollected -> 4, BlockVersion -> 1), todo)
    ...
    val expectedParameters12 = Parameters(12, Map(SoftForkStartingHeight -> 2, SoftForkVotesCollected -> 4, BlockVersion -> 1), todo)
    ...
    val expectedParameters14 = Parameters(14, Map(SoftForkStartingHeight -> 14, SoftForkVotesCollected -> 0, BlockVersion -> 1), todo)
    ...
    val expectedParameters14a = Parameters(14, Map(BlockVersion -> 1), todo)
    val esc14b = esc13.process(h14b, expectedParameters14a).get

    //wrong parameters: no vote for the fork, but parameters are there
    val wrongParameters14 = Parameters(14, Map(SoftForkStartingHeight -> 2, SoftForkVotesCollected -> 4, BlockVersion -> 1), todo)
  ...
  property("soft fork - unsuccessful voting") {
    ...
    val expectedParameters6 = Parameters(6, Map(SoftForkStartingHeight -> 2, SoftForkVotesCollected -> 3, BlockVersion -> 0), todo)
    val esc6 = esc5.process(h6, expectedParameters6).get
    ...
    val expectedParameters8 = Parameters(8, Map(SoftForkStartingHeight -> 8, SoftForkVotesCollected -> 0, BlockVersion -> 0), todo)
    ...
    val expectedParameters8e = Parameters(8, Map(BlockVersion -> 0), todo)
    val esc8e = esc7.process(h8e, expectedParameters8e).get

    // parameters are not cleared
    val wrongParameters8 = Parameters(8, Map(SoftForkStartingHeight -> 2, SoftForkVotesCollected -> 3, BlockVersion -> 0), todo)
